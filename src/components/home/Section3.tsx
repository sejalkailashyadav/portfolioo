import ConstrainedBox from "@/components/core/constrained-box";
import ResponsiveBox from "@/components/core/ResponsiveBox";
import SectionTitle from "@/components/common/SectionTitle";
import { Timeline } from "@/components/common/timeline";
import ExperienceItem from "./ui/ExperienceItem";
import EducationCard from "./ui/EducationCard"; // Make sure you have this component
import experiences from "@/data/experiences";
import education from "@/data/education";

const HomeSection3 = ({ id }: { id: string }) => {
  // Combine experiences and education into a single array
  const timelineData = [
    ...experiences.map((exp, i) => ({
      title: exp.startDate + " - " + (exp.endDate || "Present"),
      content: <ExperienceItem key={`experience-${i}`} data={exp} />,
    })),
    ...education.map((edu, i) => ({
      title: edu.startDate + " - " + (edu.endDate || "Present"),
      content: <EducationCard key={`education-${i}`} data={edu} />,
    })),
  ];

  return (
    <ResponsiveBox
      classNames="dark:bg-[var(--bgColor)] bg-[var(--bgColor)] dark:bg-grid-white/[0.1] bg-grid-white/[0.1] min-h-screen items-center justify-center"
      id={id}
    >
      <ConstrainedBox classNames="p-4 py-16">
        <SectionTitle>Experiences & Education</SectionTitle>

        <Timeline data={timelineData} />
      </ConstrainedBox>
    </ResponsiveBox>
  );
};

export default HomeSection3;
